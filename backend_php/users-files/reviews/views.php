...
if (isset($data['fileKey'])) {
    ...
    if ($conn->connect_error) {
        ...
        $sql = "UPDATE files_table SET view_count = view_count + 1 WHERE file_key = '$fileKey'";
        $result = $conn->query($sql);

        if ($result === TRUE) {
            $response['success'] = "View count updated successfully".$fileKey;
        } else {
    ...
    $response['error'] = "UserID and fileKey not received!";
    echo json_encode($response);
    exit();
}
?>
