...
if (isset($postData['user_id'], $postData['course_name'])) {
    ...
    $checkQuery = "SELECT * FROM follow_courses WHERE user_id = ? AND course_name = ?";
    ...
    if ($resultCheck->num_rows > 0) {
        ...
        $deleteQuery = "DELETE FROM follow_courses WHERE user_id = ? AND course_name = ?";
        $stmtDelete = $conn->prepare($deleteQuery);
        $stmtDelete->bind_param("ss", $userId, $courseName);
        $stmtDelete->execute();
